...
def display_lines(image , lines):
    line_of_image = np.zeros_like(image)
    if lines is not None:
        for x1 , y1 , x2 , y2 in lines:
            cv2.line(line_of_image , (x1 , y1) , (x2 , y2) , (255 , 0 , 0) , 10)
...
# line_of_image = display_lines(lane_image , averaged_lines)
#
# combo_image = cv2.addWeighted(lane_image , 0.8 , line_of_image , 1  , 1)
#
...
while(cap.isOpened()):
    ...
    line_of_image = display_lines(frame , averaged_lines)

    combo_image = cv2.addWeighted(frame , 0.8 , line_of_image , 1  , 1)
